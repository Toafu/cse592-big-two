...
class Player:
    ...
    def find_play(self, last_play: Play = None, current_combination: CardCombination = CardCombination.ANY):
        """
        ...
        match current_combination:
            case CardCombination.SINGLE:
                return [c for c in self.hand if c > last_play[0]]
...
class HumanPlayer(Player):
    def find_play(self, last_play=None):
        ...
        while True:
            play_input = input("Enter indices of cards to play (e.g., '0 2 4' for a combination): ")
            try:
                indices = list(map(int, play_input.split()))
                selected_cards = [self.hand[i] for i in indices]
                
                if is_valid_combination(selected_cards) and (not last_play or selected_cards > last_play):
                    for card in selected_cards:
                        self.hand.remove(card)
                    return selected_cards
                else:
                    print("Invalid combination or play not higher than last play.")
